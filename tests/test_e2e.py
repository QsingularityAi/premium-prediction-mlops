...
class TestEndToEndPipeline(unittest.TestCase):
    ...
    def _generate_test_data(cls):
        ...
        data = {
            ...
            'Vehicle_Type': np.random.choice(['Sedan', 'SUV', 'Truck', 'Compact'], n_samples),
            'Customer_Category': np.random.choice(['Standard', 'Premium', 'Gold'], n_samples)
        }
        ...
        data['Premium_Amount'] = np.maximum(500, premium_base + np.random.normal(0, 300, n_samples))
        ...
        df_drift['Credit_Score'] = df_drift['Credit_Score'] - 50  # Shift credit score distribution
    ...
    def test_data_processing_pipeline(self):
        ...
        # Test segmentation
        segment_data = self.data_processor.split_into_segments(X_train, y_train, premium_thresholds)
    ...
    def test_model_training_and_evaluation(self):
        ...
        # Create preprocessor
        preprocessor = self.data_processor.create_preprocessing_pipeline(numerical_features, categorical_features)
        
        # Create segments
        premium_thresholds = self.data_processor.create_segments(y_train)
        segment_data_train = self.data_processor.split_into_segments(X_train, y_train, premium_thresholds)
        test_masks = self.data_processor.get_segment_masks_test(y_test, premium_thresholds)
        
        # Define model configurations
        segment_model_configs = self.model_trainer.define_model_configs(segment_data_train)
        
        # Train models
        trained_models = self.model_trainer.train_segment_models(segment_data_train, preprocessor, segment_model_configs)
        ...
        with patch('joblib.dump') as mock_dump:
            ...
            self.assertTrue(os.path.exists(os.path.join(version_dir, 'premium_thresholds.json')))
    ...
    def test_model_deployment_and_prediction(self):
        ...
        for segment in segments:
            ...
            joblib.dump(mock_model, os.path.join(version_dir, f"{segment}_model.joblib"))
        ...
        # Test model loading
        with patch.object(model_predictor, 'load_models', return_value=True) as mock_load:
    ...
    def test_monitoring_integration(self):
        ...
        with patch('prometheus_client.Counter', return_value=MagicMock()) as mock_counter, \
             patch('prometheus_client.Histogram', return_value=MagicMock()) as mock_histogram, \
             patch('prometheus_client.Gauge', return_value=MagicMock()) as mock_gauge:
            
            # Import the API with mocked metrics
            with patch('src.models.model_predictor.ModelPredictor'):
                from src.api.app import app, prediction_counter, prediction_latency, model_errors, active_requests
    ...
    def test_data_drift_detection_and_retraining(self):
        ...
        # Ensure files exist
        self.assertTrue(os.path.exists(reference_data_path), "Reference data file not found")
        ...
        drift_detected, drifted_features = detect_drift(reference_df, drift_df, numeric_features, drift_threshold)
        ...
        self.assertIn('Credit_Score', drifted_features, "Credit_Score should be detected as drifted")
        ...
        with patch('src.train.main') as mock_train:
            ...
            retraining_triggered = trigger_retraining_if_needed(drift_detected, drifted_features)
            
            # Step 8: Verify retraining would be triggered
            self.assertTrue(retraining_triggered, "Retraining should be triggered when drift is detected")
    ...
    def test_end_to_end_model_update_process(self):
        ...
        for segment in segments:
            mock_model = MagicMock()
            mock_model.predict.return_value = np.array([1000.0])  # Initial model predicts 1000
            joblib.dump(mock_model, os.path.join(old_version_dir, f"{segment}_model.joblib"))
        
        # Save mock preprocessor
        joblib.dump(mock_preprocessor, os.path.join(old_version_dir, "preprocessor.joblib"))
        
        # Save thresholds
        premium_thresholds = {'p25': 1000.0, 'p50': 1500.0, 'p75': 2000.0, 'p95': 3000.0}
        ...
        with patch.object(predictor_old, '_determine_segment', return_value=['medium']):
            ...
            self.assertEqual(old_result['predictions'][0], 1000.0)  # Old model predicts 1000
        ...
        for segment in segments:
            ...
            mock_model_new.predict.return_value = np.array([1500.0])  # New model predicts 1500
            joblib.dump(mock_model_new, os.path.join(new_version_dir, f"{segment}_model.joblib"))
        
        # Save mock preprocessor
        joblib.dump(mock_preprocessor, os.path.join(new_version_dir, "preprocessor.joblib"))
        ...
        with patch.object(predictor_new, '_determine_segment', return_value=['medium']):
            ...
            self.assertEqual(new_result['predictions'][0], 1500.0)  # New model predicts 1500
        ...
        with patch('kubernetes.client.AppsV1Api') as mock_apps_api:
            ...
            def update_deployment_model_version(namespace, deployment_name, new_version):
