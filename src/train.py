...
def parse_args():
    ...
    parser.add_argument(
        "--model-dir", type=str, help="Override model directory from config"
    )
...
def setup_mlflow(config: Dict[str, Any], args: argparse.Namespace) -> str:
    ...
    # Set experiment name
    experiment_name = args.experiment_name or config["mlflow"].get(
...
def main():
    ...
    try:
        ...
        segment_data_train = data_processor.split_into_segments(
        ...
        with mlflow.start_run(experiment_id=experiment_id) as run:
            ...
            mlflow.log_params(
                {
                    ...
                    "feature_selection_threshold": config["training"][
            ...
            segment_model_configs = model_trainer.define_model_configs(
                segment_data_train
            )

            # Train models
            logger.info("Training segmented models...")
            trained_models = model_trainer.train_segment_models(
            ...
            if args.log_artifacts:
                ...
                plt.scatter(
                    ...
                    alpha=0.3,
                    s=10,
                    label=f'Overall R²: {combined_metrics["r2"]:.3f}',
                )
                plt.plot(
            ...
            logger.info(
                f"Training completed successfully. Models saved to {version_dir}"
            )
            ...
            for segment, metrics in segment_metrics.items():
                if metrics["count"] > 0 and metrics["r2"] is not None:
                    print(
                        f"  {segment.capitalize():<10}: R²={metrics['r2']:.4f}, RMSE={metrics['rmse']:.2f}"
                    )
