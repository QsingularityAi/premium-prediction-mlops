...
class DataProcessor:
    ...
    def __init__(self, config: Dict):
        ...
        self.test_size = config["data"][
            "test_size"
        ]  # Already correctly accessing from data section
    ...
    def preprocess_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            if "Policy_Start_Date" in df.columns:
                ...
                if orig_count > len(df):
                    logger.info(
                        f"Dropped {orig_count - len(df)} rows with invalid dates."
                    )
                ...
                df["Policy_Start_IsWeekend"] = (
                    df["Policy_Start_Date"].dt.dayofweek.isin([5, 6]).astype(int)
                )

                # Calculate days since reference date
                reference_date = pd.Timestamp("2020-01-01")
                df["Days_Since_Reference"] = (
                    df["Policy_Start_Date"] - reference_date
                ).dt.days
                ...
                logger.info(
                    "Policy_Start_Date column not found, skipping date features."
                )
        ...
        if outliers_count > 0:
            logger.info(
                f"Capping {outliers_count} outliers at 99th percentile ({p99:.2f})."
            )
            df[self.target_column] = np.where(
                df[self.target_column] > p99, p99, df[self.target_column]
            )
    ...
    def perform_feature_engineering(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(num_cols_for_eng) > 0:
            df[num_cols_for_eng] = temp_imputer_median.fit_transform(
                df[num_cols_for_eng]
            )
        else:
            logger.warning(
                "Warning: No numeric columns found for temporary imputation during FE."
            )

        # Create features
        if "Previous_Claims" in df.columns and "Insurance_Duration" in df.columns:
            df["Claims_per_Year"] = df["Previous_Claims"] / df[
                "Insurance_Duration"
            ].clip(1)
        ...
        if "Annual_Income" in df.columns:
            ...
            if "Number_of_Dependents" in df.columns:
                df["Income_per_Dependent"] = df["Annual_Income"] / df[
    ...
    def split_data(
        ...
        try:
            ...
            logger.warning(
                f"Warning: Stratified split failed ('{e}'). Using non-stratified."
            )
            X_train, X_test, y_train, y_test = train_test_split(
    ...
    def create_preprocessing_pipeline(
        ...
        categorical_transformer = Pipeline(
            steps=[
                ...
                (
                    "encoder",
                    ce.CatBoostEncoder(handle_unknown="value", handle_missing="value"),
                ),
            ]
        )
    ...
    def create_segments(self, y_train: pd.Series) -> Tuple[Dict, Dict]:
        ...
        logger.info(
            f"Thresholds: p25={p25:.2f}, p50={p50:.2f}, p75={p75:.2f}, p95={p95:.2f}"
        )

        return premium_thresholds

    def split_into_segments(
        self, X_train: pd.DataFrame, y_train: pd.Series, premium_thresholds: Dict
    ) -> Dict:
        """
        ...
        low_mask_train = (y_train > premium_thresholds["p25"]) & (
            y_train <= premium_thresholds["p50"]
        )
        medium_mask_train = (y_train > premium_thresholds["p50"]) & (
            y_train <= premium_thresholds["p75"]
        )
        high_mask_train = (y_train > premium_thresholds["p75"]) & (
            y_train <= premium_thresholds["p95"]
        )
    ...
    def get_segment_masks_test(
        self, y_test: pd.Series, premium_thresholds: Dict
    ) -> Dict:
        """
        ...
        low_mask_test = (y_test > premium_thresholds["p25"]) & (
            y_test <= premium_thresholds["p50"]
        )
        medium_mask_test = (y_test > premium_thresholds["p50"]) & (
            y_test <= premium_thresholds["p75"]
        )
