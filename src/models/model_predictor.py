...
class ModelPredictor:
    ...
    def load_models(self, version: Optional[str] = None) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Successfully loaded {len(self.segment_models)} segment models from version {version}."
            )
            return True
    ...
    def _preprocess_data(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            if "Policy_Start_Date" in X.columns:
                X["Policy_Start_Date"] = pd.to_datetime(
                    X["Policy_Start_Date"], errors="coerce"
                )
                ...
                X["Policy_Start_IsWeekend"] = (
                    X["Policy_Start_Date"].dt.dayofweek.isin([5, 6]).astype(int)
                )

                # Calculate days since reference date
                reference_date = pd.Timestamp("2020-01-01")
                X["Days_Since_Reference"] = (
                    X["Policy_Start_Date"] - reference_date
                ).dt.days
        ...
        if "Previous_Claims" in X.columns and "Insurance_Duration" in X.columns:
            X["Claims_per_Year"] = X["Previous_Claims"] / X["Insurance_Duration"].clip(
                1
            )
        ...
        if "Annual_Income" in X.columns:
            ...
            if "Number_of_Dependents" in X.columns:
                X["Income_per_Dependent"] = X["Annual_Income"] / X[
    ...
    def predict(self, X: pd.DataFrame) -> Dict:
        ...
        if medium_model is None:
            ...
            for segment, model in self.segment_models.items():
                if model is not None:
                    medium_model = model
                    logger.warning(
                        f"Medium segment model not found. Using {segment} model for preliminary prediction."
                    )
                    break
            else:
        ...
        try:
            ...
            initial_predictions = np.maximum(
                0, initial_predictions
            )  # Ensure non-negative
            ...
            for i, segment in enumerate(segments):
                ...
                if model is not None:
                    ...
                    logger.warning(
                        f"Model for segment '{segment}' not available. Using fallback model."
                    )
    ...
    def predict_batch(self, X: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            for segment in unique_segments:
                ...
                segment_predictions = np.maximum(
                    0, segment_predictions
                )  # Ensure non-negative
